...
class LoginController: UIViewController, UITextFieldDelegate{
    ...
    func SocketEventos(){
        ...
        myvariables.socket.on("LoginPassword"){data, ack in
            ...
            switch temporal[1]{
            case "loginok":
                myvariables.cliente = CCliente(idUsuario: temporal[2],idcliente: temporal[4], user: self.login[1], nombre: temporal[5],email: temporal[3], empresa: (temporal.count == 9 ? temporal[7] : "null"))
